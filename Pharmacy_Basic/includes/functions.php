...
/**
 * Get top products for a specific period with sales data
 * @param string $period Report period (today, yesterday, week, month, year, custom)
 * @param string $customStart Start date for custom period (Y-m-d)
 * @param string $customEnd End date for custom period (Y-m-d)
 * @param int $limit Number of products to return
 * @return array Top products with sales data
 */
function getTopProducts($period = 'today', $customStart = null, $customEnd = null, $limit = 5) {
    $whereClause = '';
    $params = [];
    $types = '';
    ...
    $query = "SELECT 
                ...
                SUM((oi.total_price / oi.quantity) * (oi.quantity - COALESCE(oi.quantity_returned, 0))) as total_revenue
              FROM order_items oi
    ...
    return executeQuery($query, $types, $params);
}

/**
 * Get profit data for a specific period
 * @param string $period Report period (today, yesterday, week, month, year, custom)
 * @param string $customStart Start date for custom period (Y-m-d)
 * @param string $customEnd End date for custom period (Y-m-d)
 * @return array Profit data including total cost, net profit, and profit margin
 */
function getProfitData($period = 'today', $customStart = null, $customEnd = null) {
    $whereClause = '';
    $params = [];
    $types = '';
    ...
    $query = "SELECT 
                ...
                COALESCE(SUM((oi.quantity - COALESCE(oi.quantity_returned, 0)) * oi.unit_price), 0) as total_sales,
    ...
    if ($result && isset($result[0])) {
        ...
        // Calculate net sales (total sales minus refunds)
        $netSales = $data['total_sales'] - $data['total_refunds'];
        
        // Calculate net profit (net sales minus total cost)
        $netProfit = $netSales - $data['total_cost'];
...
/**
 * Get breakdown of payment methods for a specific period
 * @param string $period Report period (today, yesterday, week, month, year, custom)
 * @param string $customStart Start date for custom period (Y-m-d)
 * @param string $customEnd End date for custom period (Y-m-d)
 * @return array Payment methods breakdown with counts and totals
 */
function getPaymentMethodsBreakdown($period = 'today', $customStart = null, $customEnd = null) {
    $whereClause = '';
    $params = [];
    $types = '';
    ...
    // Add status check to exclude returned orders
    $whereClause .= $whereClause ? " AND status != 'returned'" : "WHERE status != 'returned'";
    
    $query = "SELECT 
                payment_method,
                COUNT(*) as count,
                COALESCE(SUM(total_amount - COALESCE(refund_amount, 0)), 0) as total
              FROM orders
              $whereClause
...
/**
 * Get sales data with refunds for a specific period
 * @param string $period Report period (today, yesterday, week, month, year, custom)
 * @param string $customStart Start date for custom period (Y-m-d)
 * @param string $customEnd End date for custom period (Y-m-d)
 * @return array Sales data with refunds
 */
function getSalesDataWithRefunds($period = 'today', $customStart = null, $customEnd = null) {
    $whereClause = '';
    $params = [];
    $types = '';
    ...
    // Get comprehensive sales data including refunds
    $query = "SELECT 
                COUNT(*) as total_orders,
                COALESCE(SUM(total_amount), 0) as gross_sales,
                COALESCE(SUM(refund_amount), 0) as total_refunds,
                COALESCE(SUM(tax_amount), 0) as total_tax,
                COALESCE(AVG(CASE WHEN status != 'returned' THEN total_amount ELSE NULL END), 0) as avg_order_value
              FROM orders 
              $whereClause";
    ...
    if ($result && isset($result[0])) {
        ...
        $data['avg_order_value'] = $data['total_orders'] > 0 ? 
            $data['net_sales'] / $data['total_orders'] : 0;
